...
public abstract class RTreeNode<T extends Comparable<T>> implements Serializable{
	...
	public abstract Vector<Ref> searchSmallerEqual(T key);
	
	public abstract Vector<Ref> searchSmaller(T key);
	public abstract Vector<Ref> searchNotEqual(T key);
	public abstract Vector<Ref> searchGreaterEqual(T key);
	public abstract Vector<Ref> searchGreater(T key);
	 ...
	 */
	public abstract RPushUp<T> insert(T key, Ref recordReference, RTreeInnerNode<T> parent, int ptr);
	
	public abstract Vector<Ref> search(T key);
	public abstract Ref ceiling(T key);
	...
	public abstract boolean delete(T key, RTreeInnerNode<T> parent, Ref ref,int ptr);
	...
	public String toString()
    {
        ...
        for (int i = 0; i < order; i++)
        {
            ...
            if(i < numberOfKeys) {
                key = keys[i].toString();
                s+= key;
                if(this instanceof RTreeLeafNode) {
                    RTreeLeafNode<T> leaf = (RTreeLeafNode<T>)this;
                    s += leaf.getRecord(i) + "  ";
                }
            }
