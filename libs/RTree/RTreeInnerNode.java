...
public class RTreeInnerNode<T extends Comparable<T>> extends RTreeNode<T>  implements Serializable
{
	 ...
	 */
	public RPushUp<T> insert(T key, Ref recordReference, RTreeInnerNode<T> parent, int ptr)
	{
		int index = findIndex(key);
		RPushUp<T> RPushUp = children[index].insert(key, recordReference, this, index);
	...
	public Vector<Ref> searchSmallerEqual(T key) {
		return children[0].searchSmallerEqual( key);
		
	}
	public Vector<Ref> searchSmaller(T key){
		return children[0].searchSmaller(key);
	}
	public Vector<Ref> searchNotEqual(T key){
		return children[0].searchNotEqual(key);
	}
	public Vector<Ref> searchGreater(T key){
		return children[findIndex(key)].searchGreater(key);
	}
	public Vector<Ref> searchGreaterEqual(T key){
		return children[findIndex(key)].searchGreaterEqual(key);
	...
	public boolean delete(T key, RTreeInnerNode<T> parent,Ref ref ,int ptr) 
	{
		boolean done = false;
		for(int i = 0; !done && i < numberOfKeys; ++i)
			if(keys[i].compareTo(key) > 0)
				done = children[i].delete(key, this,ref ,i);
			
		if(!done)
			done = children[numberOfKeys].delete(key, this, ref, numberOfKeys);
		if(numberOfKeys < this.minKeys())
		{
			if(isRoot())
			{
	...
	{
		return children[findIndex(key)].ceiling(key);
	}
